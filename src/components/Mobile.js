...
function Mobile() {
    ...
    useEffect(() => {
        const interval = setInterval(() => setDate([
            moment().tz("Europe/Sarajevo").format('ddd, D. MMMM YYYY'),
            momentHijri().tz("Europe/Sarajevo").format("iD. iMMMM iYYYY").toLowerCase()
        ]), 1000);
    ...
    useEffect(() => {
        ...
        cookies.set("location", location, { path: '/', domain: '.vaktija.ba', expires: moment().add(1, "y").tz("Europe/Sarajevo").toDate() });
    }, [date, location]);
    ...
    return (
        <div>
            <table style={{ marginTop: "2px", marginBottom: "2px", marginLeft: "auto", marginRight: "auto" }}>
                <tbody>
                    <tr>
                        <th style={{ textAlign: "center", fontSize: "large" }} colSpan={2}>{locations[location]}</th>
                    </tr>
                    <tr>
                        <td colSpan={2}>{date[0]} / {date[1]}</td>
                    </tr>
