...
class CurrentDate extends Component {
    render() {
        ...
        return <p className="text-center">
            <i className={`date-${theme}`}>{date[0]} <a className={`year-${theme}`} href={`download/pdf/${location[this.props.location]}_2019.pdf`}>{date[1]}</a> / {date[2]}</i>
        </p >
    }
}
